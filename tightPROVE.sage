    ...
    for l in lines:
        ...
        if len(args) == 2:
            list_instructions.append(Instruction(
                str(args[0]), int(args[1]), int(0)))
        elif len(args) == 3:
            list_instructions.append(Instruction(
                str(args[0]), int(args[1]), int(args[2])))
        else:
    ...
    def evaluate(self, input):
        ...
        for ins in self.instructions:
            if ins.ins == 'in':
                ...
                iv[pc] = iv[ins.opA] + iv[ins.opB]
            elif ins.ins == 'and':
                iv[pc] = iv[ins.opA] * iv[ins.opB]
            elif ins.ins == 'not':
                iv[pc] = iv[ins.opA] + GF(2)(1)
            elif ins.ins == 'ref':
                iv[pc] = iv[ins.opA]
            elif ins.ins == 'out':
                iv[pc] = iv[ins.opA]
    ...
    def generate_pairs_operands(self):
        ...
        for ins in self.instructions:
            if ins.ins == 'in':
                ...
                iv[pc] = 2**(self.nb_inputs)
                self.nb_inputs += 1
                outlist.append([iv[ins.opA], iv[ins.opB]])
            elif ins.ins == 'not':
                iv[pc] = iv[ins.opA]
            elif ins.ins == 'ref':
                iv[pc] = 2**(self.nb_inputs)
...
class MultSet:
    ...
    def all_operands(self):
        list_operands = [tuple(op)
                         for m in self.mult_list for op in [m[0], m[1]]]
...
def search_attack(mult_set, verbose=False):
    ...
    if(verbose):
        print('--------------------------------------------------------')
        print('list_comb = ', list(map(boolvec_to_int,
                                       mult_set.all_operands())))
    ...
    for comb in mult_set.all_operands():
        ...
        while(1):
            ...
            match_set, mismatch_set, co_operand_list = remaining_mults.iterate(
                comb, span)
            ...
            if comb in span:
                if (verbose):
                    print('=> ATTACK')
                    print('   G:', G)
                    print('   O:', list(map(boolvec_list_to_int_list, O)))
                    print('--------------------------------------------------------')
